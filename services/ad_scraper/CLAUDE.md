...
| `purpose` values | 1, 2, 3 valid; 4+ is a parameter error. **Not nested** (TikTok/TR: 1.43M / 1.95M / 1.67M) and page 1 of a fixed filter gives near-disjoint ids, so they are different pools. It is a **gradient from in-app to web/display inventory**: AdColony 8 732 / 5 808 / 48 and Unity 2.75M / 2.43M / 2.04M fall toward 3, while AdSense 1 007 / 27 744 / 28 342 and X 19 299 / 157 688 / 304 137 rise. Those are server-side `total`s. Advertiser-mix percentages counted off pages are NOT stable — two samples gave Website 18/42/57% and 4.6/8.6/61.8%, and an earlier "Playlet concentrates under 3" claim did not survive a larger sample (zero under all three). Trust `total`, not page counts |
