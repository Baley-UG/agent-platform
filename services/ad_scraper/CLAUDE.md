...
| `purpose` values | The web app's own `purposeEnum`: **1 Game, 2 App, 3 Website**, 4 Ec, 5 Account, 6 Mainland-China games. 4 and 5 are refused with `00:401001` for this account; 6 is accepted but answers 0. Defaults to `AD_DEFAULT_PURPOSE` (2) when omitted. An earlier entry here called them a gradient from in-app to web inventory — that described the effect (game networks peak under Game, display under Website) and missed the cause: they are advertiser verticals |
...
| Required filter keys | `purpose` only (the query declares it `Int!`), and the service now defaults it to `AD_DEFAULT_PURPOSE` (2) so callers can omit it; an explicit value wins. Note Pydantic skips validators on defaults — `filters` carries `validate_default=True` or a job posted with no filters at all would reach the upstream with no purpose. `field` and `accurateSearch` are optional despite appearing in every UI-copied example. With no `isAllDate` and no range you get a default recent window: 18M vs 205M on `purpose: 2` |
...
| Mirror concurrency | Downloads go out `AD_MIRROR_CONCURRENCY` at a time (default 4), upserts stay sequential and on the event loop because a Session must not cross threads. Worth ~1.2-1.6x, not Nx — the constraint is bandwidth: 2.63 MB/s at 1 concurrent, 3.80 at 4, 2.80 at 8, measured on distinct files per level. `stats.mirror_seconds` separates download time from everything else |
